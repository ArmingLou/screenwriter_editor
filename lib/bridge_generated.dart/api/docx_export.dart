...
/// 解析Fountain文本
Future<String> parseFountainText({required String text, SimpleConf? config}) =>
    RustLib.instance.api
        .crateApiDocxExportParseFountainText(text: text, config: config);

/// 解析Fountain文本（包装版本，返回统计信息）
Future<ParseResult> parseFountainTextWithStats(
        {required String text, SimpleConf? config}) =>
    RustLib.instance.api.crateApiDocxExportParseFountainTextWithStats(
        text: text, config: config);

/// 创建默认配置
Future<SimpleConf> createDefaultConfig() =>
    RustLib.instance.api.crateApiDocxExportCreateDefaultConfig();

/// 创建中文配置
Future<SimpleConf> createChineseConfig() =>
    RustLib.instance.api.crateApiDocxExportCreateChineseConfig();

/// 创建英文配置
Future<SimpleConf> createEnglishConfig() =>
    RustLib.instance.api.crateApiDocxExportCreateEnglishConfig();

/// 导出DOCX文档为Base64编码（用于预览）
Future<ExportResult> exportToDocxBase64(
        {required String text, SimpleConf? config}) =>
    RustLib.instance.api
        .crateApiDocxExportExportToDocxBase64(text: text, config: config);
